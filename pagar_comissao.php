...
// $portabilidade = $comissao['portabilidade'];

$select_companhia = "SELECT * FROM tbl_finalizado where id = '$txt_id_finalizado'";
$result_companhia = mysqli_query($conect, $select_companhia) or die (mysql_error());
if ($rs_companhia = mysqli_fetch_array($result_companhia)){
    $portabilidade = $rs_companhia['portabilidade'];
}
...
$insert_transacoes = "INSERT INTO tbl_transacoes(data,descricao,id_origem,id_destino,valor, id_finalizado, parcela, id_transacao) values ( '$data' , '$descricao' , '$id_origem' , '$id_destino' , '$valor_calc', '$txt_id_finalizado', '$txt_parcela', '$id_transacao') ";
    ...
    while ($contador < count($usuario)){
        ...
        if ($rs = mysqli_fetch_array($result)){
            ...
            while ($rs = mysqli_fetch_array($result)){
                ...
                if ($corretor == 1){
                    $transacao = "INSERT INTO tbl_transacoes(data,descricao,id_origem,id_destino,valor, id_finalizado, parcela, vendedor, id_transacao, id_bruto, irrf) values ( '$data', '$descricao $descricao_comissao' , '$id_destino' , '$id_conta_insert' , '".$valor_calc_liquid."', '$txt_id_finalizado', '$txt_parcela', '1', '$id_transacao', '$id_bruto', '$irrf') ";

                    mysqli_query($conect, $transacao) or die(mysqli_error($conect));
                }else{
                    $transacao = "INSERT INTO tbl_transacoes(data,descricao,id_origem,id_destino,valor, id_finalizado, parcela, id_transacao, id_bruto, irrf) values ( '$data', '$descricao $descricao_comissao' , '$id_destino' , '$id_conta_insert' , '".$valor_calc_liquid."', '$txt_id_finalizado', '$txt_parcela', '$id_transacao', '$id_bruto', '$irrf') ";
